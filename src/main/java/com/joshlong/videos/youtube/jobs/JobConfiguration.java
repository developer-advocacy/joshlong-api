...
class JobConfiguration {
	...
	/**
	 * run the various jobs to ingest videos and then be schedule tweets promoting them
	 */
	private void doRunIngestJob(IngestJob ingest, PromotionJob promotion) {
		...
		var both = justIngest//
				.thenMany(justPromotion)//
				.doOnError(ex -> log.error("something's gone wrong!", ex));
		both.subscribe();
	}
