...
class DefaultBlogPostService implements BlogPostService {
    ...
    private BlogPost buildBlogPostFrom(BlogPostContentType type, String path, String contents) {
        ...
        var uniquePath = path.toLowerCase(Locale.ROOT).startsWith("/jl/blogpost/") ?   path.substring("/jl/blogpost/".length()) : path ;
        return new BlogPost(header.get("title"), date, contents, processedContent, published, type, path, uniquePath, images,
                firstParagraph);
    }
